module SubscriptionPolicy
  ...
  def has_subscription_feature?(feature_name)
    ...
    @account.has_subscription_feature?(feature_name)
  end
  ...
  # Require administrator role AND subscription feature access
  ...
  def current_subscription_tier
    return 'basic' unless @account

    @account.subscription_tier.to_s
  end

  # Get required tier for a feature
  def required_tier_for(feature_name)
    return 'premium' unless @account

    @account.required_tier_for_feature(feature_name)
  end
end
